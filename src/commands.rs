use crate::puzzle_state::{Reorientation, Twist};
...
pub enum Command {
    ...
    /// Reorient the whole puzzle state (every piece), animated through the
    ...
    /// history as the induced reorientation, so undoing past an align
    /// rotates the puzzle back in view like any other reorientation. The
    /// only command whose meaning depends on the view.
    Align,
    Undo,
    Redo,
